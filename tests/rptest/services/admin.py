...
class OutboundDataMigration:
    ...
    def __init__(self, topics: list[NamespacedTopic], consumer_groups: list[str]):
...
class Admin:
    ...
    def _request(
        ...
        **kwargs: Any,
        ...
        if r.status_code != 200:
            ...
            if "application/json" in r.headers.get("Content-Type") and len(r.text):
                try:
    ...
    def is_sample_license(resp) -> bool:
        ...
        return (
            resp is not None
            and resp.get("license", None) is not None
    ...
    def get_partitions(self, topic=None, partition=None, *, namespace=None, node=None):
        """
    ...
    def mark_transaction_expired(self, topic, partition, pid, namespace, node=None):
        """
    ...
    def delete_partition_from_transaction(
        self, tid, namespace, topic, partition_id, etag, node=None
    ):
        """
    ...
    def cancel_partition_move(self, topic, partition, namespace="kafka", node=None):
        path = f"partitions/{namespace}/{topic}/{partition}/cancel_reconfiguration"
        return self._request("post", path, node=node)

    def force_abort_partition_move(
        self, topic, partition, namespace="kafka", node=None
    ):
        path = (
    ...
    def get_majority_lost_partitions_from_nodes(
        self, dead_brokers: list[int], node=None, **kwargs
    ):
        assert dead_brokers
    ...
    def partition_transfer_leadership(
        self, namespace, topic, partition, target_id=None
    ):
    ...
    def get_cpu_profile(self, node: MaybeNode = None, wait_ms: int | None = None):
        """
    ...
    def get_local_offsets_translated(
        self, offsets, topic, partition, translate_to="kafka", node=None
    ):
        """
    ...
    def get_cloud_storage_anomalies(self, namespace: str, topic: str, partition: int):
        return self._request(
    ...
    def get_tx_manager_recovery_status(self, node: Optional[ClusterNode] = None):
    ...
    def override_node_id(
        self, node, current_uuid: str, new_node_id: int, new_node_uuid: str
    ):
        return self._request(
            "PUT",
            "debug/broker_uuid",
    ...
    def transforms_gc_committed_offsets(self, node: Optional[ClusterNode] = None):
    ...
    def get_data_migration(self, migration_id: int, node: Optional[ClusterNode] = None):
    ...
    def unsafe_abort_group_transaction(
        ...
        return self._request(
            "POST", f"transaction/unsafe_abort_group_transaction/{group_id}?{params}"
        )

    def put_ctracker_va_message(self, shard: int, msg: str, node: MaybeNode = None):
        params = {"message": msg}
        return self._request(
