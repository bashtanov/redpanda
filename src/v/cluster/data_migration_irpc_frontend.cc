...
irpc_frontend::get_entities_status(id migration_id, bool include_groups) {
    if (!_features.local().is_active(
          features::feature::migration_topic_wise_approval)) {
        return ssx::now(result<entities_status, errc>{errc::feature_disabled});
    }
    return _backend.invoke_on_instance(
      &backend::get_entities_status, migration_id, include_groups);
}
...
irpc_frontend::set_entities_status(id migration_id, entities_status status) {
    if (!_features.local().is_active(
          features::feature::migration_topic_wise_approval)) {
        return ssx::now(errc::feature_disabled);
    }
    return _backend.invoke_on_instance(
