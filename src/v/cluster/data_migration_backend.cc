...
void backend::approve_and_schedule_topic_work(
  topic_map_t::reference topic_entry) {
    ...
    if (!std::exchange(tstate.approved_to_start, true)) {
        vlog(dm_log.debug, "topic {} approved to start, scheduling work", nt);
...
backend::do_topic_work(model::topic_namespace nt, topic_work tw) noexcept {
    ...
    try {
        ...
        ec = co_await std::visit(
          [this, &nt, &tw, &tsws](const auto& info) mutable {
              return do_topic_work(nt, tw.sought_state, info, tsws);
          },
          tw.info);
        vlog(
          dm_log.debug,
...
backend::build_migration_reconciliation_state(
